...
"Drop packets using eBPF — Write an eBPF code to drop the TCP packets on a port (def: 4040). Additionally, if you can make the port number configurable from the userspace, that will be a big plus."
...
# example using an interface name:
sudo ./tcp_drop_user enp0s3 4040
```

If you're testing across VM/host, run `hostname -I` inside the VM to get its IP address and use that from the host so the XDP program sees packets on the interface.
...
# Run client
./tcp_drop_user <server-ip> 4040  # should succeed
./tcp_drop_user <server-ip> 8080  # should be dropped if configured
```
...
"Drop packets only for a given process — Write an eBPF code to allow traffic only at a specific TCP port (default 4040) for a given process name (for e.g, \"myprocess\"). All the traffic to all other ports for only that process should be dropped."
...
```sh
sudo mkdir -p /sys/fs/cgroup/mygrp
# (Optional) put a process into the cgroup manually:
echo <pid> | sudo tee /sys/fs/cgroup/mygrp/cgroup.procs
...
```sh
# Start a server on allowed port
python3 problem-2/tcp_server.py 4040

# Run your client inside the cgroup
./myprocess <server-ip> 4040
```
...
```sh
sudo bpftool cgroup detach /sys/fs/cgroup/mygrp cgroup_inet4_connect all
sudo rm /sys/fs/bpf/filter_traffic
sudo rmdir /sys/fs/cgroup/mygrp
```
